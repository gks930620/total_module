> ✅ 이 문서의 결론(루트 단일 Dockerfile + APP_MODULE로 서비스별 실행)은 **2026-07-08 방향 결정 이후에도 그대로 유효**하다.
> 단, 여기서 "모듈러 모놀리스"는 "저장소·이미지 통합"을 뜻하며, 실행은 모듈별 서비스로 분리한다 ([`아키텍처_방향결정.md`](아키텍처_방향결정.md)).
