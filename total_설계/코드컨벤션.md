...
### 5-3. 파일 저장 — **DB(LONGBLOB) 표준, 외부 스토리지 안 씀** ⭐
- 업로드 파일(이미지 등) 바이트는 **DB(`stored_files` 등 LONGBLOB) 에 저장**한다. 로컬(H2)·운영(MySQL) 동일.
- **Supabase·S3 등 외부 오브젝트 스토리지는 쓰지 않는다** (2026-07-10 확정: 모든 프로젝트가 Railway MySQL을 가지므로 외부 의존성 불필요).
- 컨테이너 로컬 디스크 저장 금지(휘발성). 서빙은 `/uploads/{저장파일명}` 등 컨트롤러가 DB에서 읽어 스트리밍.
- businesscard_qr·doll_gacha 모두 이 방식(둘 다 `stored_files` LONGBLOB).
