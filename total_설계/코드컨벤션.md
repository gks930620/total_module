...
## 5. 배포(Railway) 관련 (요약 — 상세는 아키텍처_방향결정.md)

- 모든 운영 설정은 환경변수로: `${ENV_NAME:로컬기본값}`. 비밀값은 소스/커밋에 두지 않는다.
- `GET /healthz` 필수(200 + `{status:UP}`), 헬스체크 회귀 방지 스모크 테스트 1건 유지.
- 절대 URL은 요청 헤더에서 유추하지 말고 공개 base URL 환경변수 기준으로 생성.
- 파일 저장은 컨테이너 로컬 디스크 금지(휘발성) → DB/오브젝트 스토리지.
- 운영에서 인메모리 DB로 부팅되면 fail-fast(데이터 소실 방지).

---

## 6. 테스트

- 통합 테스트로 주요 흐름 + 에러 경로(권한 거부/검증 실패/404)를 커버.
