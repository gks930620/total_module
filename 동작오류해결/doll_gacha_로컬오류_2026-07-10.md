...
### 추가 조치 — 로컬 DB를 휘발성으로 전환 (수동 삭제 불필요하게)
처음엔 "H2 파일을 지우고 재시작"을 안내했으나, 아예 **로컬 DB를 인메모리로 바꿔** 매 기동 시
스키마를 새로 만들고 시드를 다시 로드하도록 했다. 이제 **손으로 지울 필요 없이** 재시작만 하면
항상 UTF-8 시드가 깨끗하게 로드된다. (설계 원칙으로 승격 → [`total_설계/코드컨벤션.md`](../total_설계/코드컨벤션.md) §5-2)
- `application.yml`: `jdbc:h2:mem:...` + `ddl-auto: create` + `sql.init.encoding: UTF-8`
- 즉 **로컬은 데이터 비영속(매번 초기화), 운영은 영속** 이라는 원칙.
- 기존에 `doll_gacha/data/` 파일 DB가 남아 있으면 지워도 된다(이제 안 씀).
