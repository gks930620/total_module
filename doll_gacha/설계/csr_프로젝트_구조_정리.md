...
> ⚠️ **최신화 안내 (2026-07-06 기준 실제 코드와 정정)** — 이 문서는 과거 보일러플레이트(`com.test.test`, 채팅 포함) 기준으로 작성되어 현재 구현과 다릅니다. 실제 기준은 아래와 같습니다:
> - **패키지**: `com.test.test` ❌ → 실제 `com.doll.gacha` ✅
> - **화면 서빙**: Thymeleaf ❌ → **React SPA**(Vite) 로 완전 전환, `templates/` 삭제됨. `HomeController` 가 `forward:/index.html`
> - **실시간 채팅(STOMP/WebSocket, `/rooms`, `RoomController`, `stomp/`)**: **미구현**(향후 확장). 본 문서의 채팅 섹션·"구현 현황 ✅ 채팅"은 사실이 아님
> - **실제 핵심 도메인**: **`dollshop`(인형뽑기 매장) + `review`(매장 리뷰)** — 본 문서에 누락됨
> - **REST 경로 정정**: `/api/communities` → `/api/community`, `/api/users/me` → `/api/my/info`, `/api/tokens/refresh` → `/api/refresh/reissue`, `/api/rooms/**` → (없음)
>
> 아래 원문은 이력 보존용이며, 위 정정 내용이 우선합니다.
